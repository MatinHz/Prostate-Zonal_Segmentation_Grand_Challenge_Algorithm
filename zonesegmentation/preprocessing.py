...
def getROI(inputDir):
    ...
    # save image
    sitk.WriteImage(tra_orig_roi, os.path.join(inputDir, 'roi_tra.nii.gz'))

    return tra_orig_roi

def getROI_seperate_folders(inputDir):

    files = os.listdir(os.path.join(inputDir, 'transverse-t2-prostate-mri'))
    for file in files:
        # load images
        if 'tra' in file or 'mha' in file:
            img_tra = sitk.ReadImage(os.path.join(inputDir, 'transverse-t2-prostate-mri', file))
            
    files = os.listdir(os.path.join(inputDir, 'coronal-t2-prostate-mri'))
    for file in files:
        # load images
        if 'cor' in file or 'mha' in file:
            img_cor = sitk.ReadImage(os.path.join(inputDir, 'coronal-t2-prostate-mri', file))
            
    files = os.listdir(os.path.join(inputDir, 'sagittal-t2-prostate-mri'))
    for file in files:
        # load images
        if 'sag' in file or 'mha' in file:
            img_sag = sitk.ReadImage(os.path.join(inputDir, 'sagittal-t2-prostate-mri', file))
...
def preprocessImage(imgDir):
    ...
    roi_tra = getROI_seperate_folders(imgDir)
